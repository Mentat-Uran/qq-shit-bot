...
def test_mac_config_routes_image_to_sensenova_then_text_to_deepseek():
    ...
    assert defaults["model"] == {
        ...
        "fallbacks": ["sensenova-token/deepseek-v4-flash"],
    }
...
def test_mac_shell_entries_are_unix_only_and_use_mac_compose():
    ...
    assert {script.name for script in scripts} >= {"start.sh", "stop.sh", "status.sh", "logs.sh", "check-env.sh", "console.sh"}
...
def test_mac_environment_template_has_explicit_lan_controls_without_real_values():
    ...
    for key in ("OPENCLAW_GATEWAY_BIND_HOST", "OPENCLAW_GATEWAY_PUBLIC_HOST", "OPS_CONSOLE_BIND_HOST", "OPS_CONSOLE_PORT", "OPS_CONSOLE_TOKEN", "SENSENOVA_VISION_MODEL"):
