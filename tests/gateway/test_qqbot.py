...
class TestWaitForReconnection:
    ...
    @pytest.mark.asyncio
    async def test_send_swallows_runtime_diagnostics_before_network(self):
        adapter = self._make_adapter(app_id="a", client_secret="b")
        adapter._running = False
        adapter._wait_for_reconnection = mock.AsyncMock(return_value=False)

        diagnostics = [
            "⚡ Interrupting current task (iteration 3/60, running: web_search).",
            "⚠️ The model declined to respond to this request (safety refusal).",
            "API call failed after 3 retries: HTTP 429",
        ]
        for diagnostic in diagnostics:
            result = await adapter.send("test_openid", diagnostic)
            assert result.success

        adapter._wait_for_reconnection.assert_not_awaited()
