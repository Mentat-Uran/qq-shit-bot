...
class TestBusySessionAck:
    ...
    @pytest.mark.asyncio
    async def test_qq_interrupts_and_queues_without_sending_busy_ack(self):
        runner, sentinel = _make_runner()
        runner._busy_input_mode = "interrupt"
        runner._queued_events = {}
        adapter = _make_adapter(platform_val="qqbot")

        event = _make_event(text="new choice", platform_val="qqbot")
        sk = build_session_key(event.source)
        agent = MagicMock()
        runner._running_agents[sk] = agent
        runner.adapters[event.source.platform] = adapter
        ...
        agent.interrupt.assert_called_once_with("new choice")
        assert adapter._pending_messages[sk] is event
        adapter._send_with_retry.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_qq_draining_status_is_silent(self):
        runner, sentinel = _make_runner()
        ...
        event = _make_event(text="keep this", platform_val="qqbot")
        sk = build_session_key(event.source)
        runner.adapters[event.source.platform] = adapter
