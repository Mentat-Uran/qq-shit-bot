...
async def vision_analyze_tool(
    ...
    try:
        ...
        try:
            ...
            _vision_native_ollama = str(_vision_cfg.get("api_mode") or "").strip().lower() == "ollama"
        ...
        try:
            if _vision_native_ollama:
                response = await _call_ollama_native_vision(
                    image_data_url=image_data_url,
...
async def _call_ollama_native_vision(
    *,
    ...
    """Call Ollama's native multimodal API for local vision backends.

    Ollama's OpenAI-compatible endpoint does not accept the ``image_url``
    content-part shape used by Hermes. Its native API accepts the same image
    bytes as a base64 string in ``messages[].images``.
    """
    ...
    model_name = str(vision_cfg.get("model") or "qwen2.5vl:7b").strip()
    if base_url.endswith("/v1"):
        base_url = base_url[:-3].rstrip("/")
    endpoint = f"{base_url}/api/chat"
    if not base_url:
        raise RuntimeError("Ollama vision endpoint is not configured")
