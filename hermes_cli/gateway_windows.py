...
def _build_gateway_cmd_script(
    ...
    # Keep the SenseNova/official-DeepSeek failover watcher attached to every
    # Windows Gateway entry point, including the Scheduled Task wrapper.  The
    # watcher owns a machine-wide mutex, so this is safe when the desktop
    # launcher and the task are both invoked around the same time.
    lines.append(r'set "SENSENOVA_WATCHER=%USERPROFILE%\Desktop\bat\watch-sensenova-v4-recovery.ps1"')
    lines.append(
        r'''if exist "%SENSENOVA_WATCHER%" powershell.exe -NoLogo -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -Command "Start-Process -FilePath 'powershell.exe' -WindowStyle Hidden -ArgumentList @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File','%SENSENOVA_WATCHER%','-PollSeconds','60')"'''
    )
