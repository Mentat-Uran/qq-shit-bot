...
_QQ_RUNTIME_DIAGNOSTIC_PREFIXES = (
...
def _is_qq_runtime_diagnostic(content: str) -> bool:
    """Return True for Hermes operational/error text that must stay out of QQ."""
    text = str(content or "").lstrip()
    text = text.lstrip("⚡⏳⏩⚠️\ufe0f ").lower()
    return any(text.startswith(prefix) for prefix in _QQ_RUNTIME_DIAGNOSTIC_PREFIXES)
...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._full_group_message_event_seen = False
        self._proactive = ProactiveGroupManager(
            ProactiveConfig.from_extra(extra),
            send_callback=self._send_proactive_message,
        )
        ...
        # QQ may deliver the same group message through both the full-message
        # and @-message dispatches.  Keep the route selected for that ID so a
        # late explicit event can recover a message that was first ingested as
        # context, without replying twice when the full event already handled
        # the @-message.
        self._seen_message_routes: Dict[str, str] = {}
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._write_proactive_status()
            if self._proactive.config.enabled:
                logger.info(
                    "[%s] QQ proactive group chat enabled; waiting for GROUP_MESSAGE_CREATE "
                    "permission/event before autonomous participation",
                    self._log_tag,
                )
    ...
    def set_proactive_callbacks(
        self,
        *,
        decision_callback: Optional[Callable[[ProactiveSnapshot], Awaitable[Any]]],
        reply_callback: Optional[Callable[[ProactiveSnapshot, Any], Awaitable[Optional[str]]]],
    ) -> None:
        """Attach Gateway model callbacks without changing normal @ handling."""
        self._proactive.set_callbacks(
    ...
    def proactive_status(self) -> Dict[str, Any]:
        """Return a safe operator-facing status for autonomous group intake."""
        details = self._proactive.status()
        details.update({
            "full_group_message_event_seen": self._full_group_message_event_seen,
            "available": bool(details.get("enabled") and self._full_group_message_event_seen),
        })
        if not details.get("enabled"):
            details["reason"] = "disabled_by_config"
        elif not self._full_group_message_event_seen:
            details["reason"] = "waiting_for_GROUP_MESSAGE_CREATE_permission"
        elif not details.get("callbacks_configured"):
            details["reason"] = "gateway_model_callbacks_not_configured"
        else:
            details["reason"] = "available"
        return details

    def _write_proactive_status(self) -> None:
        """Publish capability state without exposing credentials or chat text."""
        try:
            self._write_runtime_status_safe(
                "proactive",
                platform_details={"proactive_group_chat": self.proactive_status},
            )
        except Exception:
            logger.debug("[%s] Failed to publish proactive status", self._log_tag, exc_info=True)
    ...
    async def _send_proactive_message(
        ...
        """Send proactive output through the same official QQ REST path."""
        result = await self.send(snapshot.group_id, content, reply_to=reply_to)
        if getattr(result, "success", False):
            self._last_msg_id[snapshot.group_id] = (
                getattr(result, "message_id", None)
                or self._last_msg_id.get(snapshot.group_id, "")
            )
        return result

    @staticmethod
    def _summarize_attachments(attachments: Any) -> Dict[str, Any]:
        """Describe full-mode attachments without downloading them.

        Full group intake must remain cheap because every group message can
        arrive through ``GROUP_MESSAGE_CREATE``.  Explicit @ messages still
        use the existing attachment download/transcription path below.
        """
        ...
        for item in attachments[:10]:
            if not isinstance(item, dict):
                continue
            kind = str(item.get("content_type") or item.get("type") or "attachment").strip()
            filename = str(item.get("filename") or "").strip()
            labels.append(f"{kind}:{filename}" if filename else kind)
        if not labels:
            return {
                "image_urls": [],
                "image_media_types": [],
        ...
        suffix = " …" if len(attachments) > len(labels) else ""
        return {
            "image_urls": [],
            "image_media_types": [],
            "voice_transcripts": [],
            "attachment_info": "[附件: " + ", ".join(labels) + suffix + "]",
        }

    @classmethod
    def _quoted_text_only(cls, d: Dict[str, Any]) -> str:
        """Extract quoted text/attachment markers without remote I/O."""
        try:
        ...
        for elem in elements[:10]:
            ...
            attachment_info = cls._summarize_attachments(elem.get("attachments"))["attachment_info"]
            if attachment_info:
    ...
    async def _on_message(self, event_type: str, d: Any) -> None:
        ...
        msg_id = str(d.get("id", ""))
        is_duplicate = bool(msg_id) and self._is_duplicate(msg_id)
        allow_late_at = (
            is_duplicate
            and event_type == "GROUP_AT_MESSAGE_CREATE"
            and self._seen_message_routes.get(msg_id) == "full_message"
        )
        if not msg_id or (is_duplicate and not allow_late_at):
            logger.debug(
        ...
        if event_type == "C2C_MESSAGE_CREATE":
            ...
            full_message = event_type == "GROUP_MESSAGE_CREATE"
            # With the full-message permission enabled, an @ message can
            # arrive first as GROUP_MESSAGE_CREATE.  Route that case through
            # the existing explicit-reply path; otherwise the later
            # GROUP_AT_MESSAGE_CREATE event is hidden by message de-duplication.
            if full_message and self._is_group_bot_mention(d, content):
                full_message = False
            self._seen_message_routes[msg_id] = (
                "full_message" if full_message else "explicit"
            )
    ...
    async def _handle_group_message(
        ...
        """Handle an official QQ group @ or full-message event."""
        group_openid = str(d.get("group_openid", ""))
        if not group_openid:
            return
        if full_message:
            self._full_group_message_event_seen = True
            self._write_proactive_status()
        ...
        text = content if full_message else self._strip_at_mention(content)
        if full_message:
            att_result = self._summarize_attachments(d.get("attachments"))
        else:
        ...
        # Full-message intake is intentionally lightweight: it keeps quoted
        # text and attachment markers but does not download every group file
        # before the participation decision. Explicit @ messages retain the
        # existing rich attachment/quote path.
        if full_message:
            text = self._merge_quote_into(text, self._quoted_text_only(d))
        else:
            quoted = await self._process_quoted_context(d)
            text = self._merge_quote_into(text, quoted["quote_block"])
            if quoted["image_urls"]:
        ...
        event = MessageEvent(
            source=self.build_source(
                ...
                user_name=str(author.get("username", "")) or None,
                chat_type="group",
        ...
        if full_message:
            if self._proactive.config.enabled:
                await self._proactive.ingest(event)
            return
        if self._proactive.config.enabled:
            await self._proactive.ingest(event, explicit=True)
        await self.handle_message(event)
    ...
    async def send(
        ...
        # Final transport-level defense: operational diagnostics have several
        # call paths in the gateway. Treat them as successfully swallowed so a
        # future direct-send branch cannot leak English errors into QQ chat.
        if _is_qq_runtime_diagnostic(content):
            logger.warning("[%s] Suppressed QQ runtime diagnostic", self._log_tag)
    ...
    def _strip_at_mention(content: str) -> str:
        ...
        stripped = re.sub(
            r"^\s*(?:<@!?[^>]+>|@\S+)\s*", "", content.strip()
        )
        return stripped

    def _is_group_bot_mention(
            self, d: Dict[str, Any], content: str
    ) -> bool:
        """Detect a bot @ in a full group-message payload.

        QQ deployments differ in whether the mention is preserved in
        ``content`` or represented in message-element metadata.  Keep the
        text fallback narrow and only accept structured mention targets that
        match this bot's app ID.
        """
        import re

        # Full-message payloads can arrive without a separate
        # GROUP_AT_MESSAGE_CREATE event, while preserving the raw QQ mention
        # token.  Treat a leading mention as the explicit fallback; the
        # explicit path strips the token before the model sees the message.
        if re.match(r"^\s*(?:<@!?[^>]+>|@\S+)", str(content or "")):
            return True

        app_id = self._app_id.strip()
        if not app_id:
            return False

        mention_markers = {"at", "mention", "text_mention", "user_mention"}
        target_keys = {
            "id",
            "user_id",
            "openid",
        ...
        def walk(value: Any, *, mention_context: bool = False) -> bool:
            if isinstance(value, list):
                return any(walk(item, mention_context=mention_context) for item in value)
            ...
            current_context = mention_context or marker in mention_markers
            if current_context:
            ...
            for key, child in value.items():
                child_context = current_context or str(key).lower() in {
                    "mention",
                    "mentions",
                    "mention_users",
                    "at",
                ...
                if walk(child, mention_context=child_context):
                    return True
            return False

        for key in ("mentions", "mention", "mention_users", "msg_elements", "elements"):
            if walk(d.get(key), mention_context=key in {"mentions", "mention", "mention_users"}):
                return True
        return False
    ...
    def _is_duplicate(self, msg_id: str) -> bool:
        ...
        if len(self._seen_messages) > DEDUP_MAX_SIZE:
            ...
            self._seen_message_routes = {
                key: route
                for key, route in self._seen_message_routes.items()
