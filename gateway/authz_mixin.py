...
class GatewayAuthorizationMixin:
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        # A QQ-specific deny list must take precedence over the group-wide
        # allowlist.  QQ_GROUP_ALLOWED_USERS=* otherwise authorizes every
        # group member before the normal per-user checks run.
        if source.platform == Platform.QQBOT and user_id:
            blocked_qq_users = {
                value.strip()
                for value in os.getenv("QQ_BLOCKED_USERS", "").split(",")
                if value.strip()
            }
            if user_id in blocked_qq_users:
                logger.warning("Blocked QQ user: %s", user_id)
                return False
