...
| 群聊小游戏与信息差 | `qqbot-game` 的 `chat_games.py`、QQ 交互 patch、海龟汤 adapter | 定向 Python 游戏/部署测试：11 passed；Node 交互测试：6 passed；容器 API 确认成语库 29,534 条，海龟汤启动/状态响应均不含 `title` | 镜像与 loopback API 通过不等于真实 QQ 收发；进行中的游戏仍是内存态 |
