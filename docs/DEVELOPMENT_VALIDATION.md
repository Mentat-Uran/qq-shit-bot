...
| CI 重构 | `.github/workflows/openclaw-validation.yml` | Compose `config --quiet` 通过；CI 已拆为 deployment/Compose/JavaScript/PowerShell/shell/hygiene 六个职责 job，无重复测试矩阵 | GitHub runner 检查不等于线上服务 |
...
| 重型视觉代码处置与文档同步 | `docs/retired-visual/`、README/部署文档 | 旧 Compose/源码已归档，活动 Compose 无 video/image-fusion 引用；部署、维护、安全和 Windows 文档已同步 | 历史材料不属于当前部署 |

补充本地结果：`python -m pytest tests/deploy -q` 为 `11 passed, 1 skipped`（Windows 主机无 Unix shell 的迁移测试按平台跳过）；`git diff --check` 通过；PowerShell AST 通过；Docker daemon 可用且 Compose 形状通过，但未启动或拉取运行镜像。
