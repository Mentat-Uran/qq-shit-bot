...
| `fei98/civil-service-exam-prep` 题库快照 | `questions/questions.json` 按 `tools/import_exam_bank.py` 导入；当前本地保留 4148 条文字题 | `qqbot-game` 的行测随机抽题、答案、解析和确定性判分；必须依赖图片/图表的题不进入纯文字路径 | 上游应用代码仓库声明 MIT；其 README 说明真题数据来自 `mpbfx/gongkao`（AGPL-3.0），题面/解析版权归原作者；本地使用与再分发边界须由部署者另行核对 |
