...
本目录的文字游戏面向 QQ 群聊设计：保留成语接龙和共享答案的猜成语，并由
`structured_games.py` 扩展数字炸弹、24 点、猜人物、猜作品、知识抢答、真假判断、
找不同、词语分类、一句话推理、脑筋急转弯、谜语、飞花令、诗词接龙、排序题、线索竞拍
和行测抢答。规则适配层不依赖 QQ、模型或文件系统，因此可以用固定 fixture 单独测试。
进行中的房间只保存在 `qqbot-game` 进程内，侧车重启后清空；不会把群号、玩家 OpenID、
猜测或答案写入持久化状态目录。
...
位置，不扣猜测次数，重复提交不扣次数。其他游戏的玩家、房间、积分、空闲过期和
答案统计由 `StructuredGameManager` 统一管理，题库在 `game_bank.json` 和
`exam_bank.json` 中独立维护。

## 行测题库来源

行测题库导入脚本位于仓库根目录的 `tools/import_exam_bank.py`。它不联网、不调用
LLM，只接受明确指定的 `questions/questions.json`，保留可结构化判分的文字题，并
排除必须依赖图片、图表或示意图的题目。完整导入命令和来源/许可证说明见
[`docs/QQBOT_CHAT_GAMES.md`](../../../../docs/QQBOT_CHAT_GAMES.md)。
